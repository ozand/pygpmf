...
## Версия 0.3.0 (12 января 2026)

### Новые возможности
- 🚀 **Поддержка Hero 11-13 (GPS9, 10Hz)**: `extract_gps_blocks()` и `parse_gps_block()` автоматически распознают GPS5/GPS9, корректно парсят 9-польную структуру GPS9 (lat/lon/alt/speed2d/speed3d/days/seconds/dop/fix) с обратной совместимостью по первым 5 полям
- 🛰️ **Новый модуль гироскопа/акселерометра**: `gpmf.gyro` с контейнерами `GyroData` и `AccelData`, парсингом GYRO/ACCL, генераторами блоков и заделом для экспорта в GyroFlow
- 📑 **Примеры для Hero 11-13**: `examples/example_gps9_extract.py` и `examples/example_gyro_extract.py` демонстрируют извлечение GPS9 и IMU-данных

### Тестирование
- ✅ **134 теста проходят** (добавлено 15 новых: 7 для GPS9, 8 для GYRO/ACCL)
- ✅ **Покрытие кода: 79.77%** (gps.py: 83.33%, gyro.py: 48.00%)
- ✅ Поддержка Python 3.9–3.13 на Windows/Linux/macOS

### Обратная совместимость
- ✔️ GPS5 (Hero 5-10) работает без изменений, автоматически выбирается при отсутствии GPS9
- ✔️ Публичные API не ломаются; добавлен экспорт модуля `gyro` в `gpmf.__init__`

---

## Версия 0.2.1 (12 января 2026)

### Исправления
- 🐛 **Критический фикс**: Защищенное декодирование FourCC в `parse.iter_klv`
  - Исправлена UnicodeDecodeError при работе с бинарными GPMF данными
  - Добавлен fallback на latin-1 для не-ASCII FourCC кодов
  - Добавлена защита от неполных заголовков в KLV stream

### Тестирование
- ✅ **Покрытие тестами: 83.67%** (119 тестов проходят)
- ✅ Добавлен регрессионный тест для не-ASCII FourCC
...
- 📖 Добавлены badges: PyPI, Python versions, Documentation, Coverage
...
## Изменения версии 0.2.1

- Исправлено декодирование FourCC в `parse.iter_klv` (без ошибок UTF-8 на бинарных значениях)
- Добавлен регресcионный тест `test_iter_klv_non_ascii_fourcc`
- Покрытие тестами повышено до ~83.6%
